...
def is_key_value_in_name_space(name_space, key_1, value_1):
    ...
    for key_in_space, values_in_space in name_space.items():
        ...
        if key_1 == key_in_space and value_1 in values_in_space:
            # print('строка 11,', 'key:', key, 'value:', value,
            #       'есть такая же пара ключ значение')
            return 'Yes'  # если есть то просто возвращаем Yes
...
for _i in range(count_answers):
    ...
    if len(input_check) > 1:
        ...
        print(is_key_value_in_name_space(
            input_name_space, input_check[0], input_check[0]))
